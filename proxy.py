...
[ nebbbes : https://github.com/nebbbes]
""" + bcolors.ENDC)
...
def backup():
    try:
        shutil.copy('good_proxy.txt', 'ProxyBackup.txt')
        print(bcolors.WARNING +
              'good_proxy.txt backed up in ProxyBackup.txt' + bcolors.ENDC)
    except Exception:
        pass

    print('', file=open('good_proxy.txt', 'w'))
...
def main_checker(proxy_type, proxy, position):
    ...
    try:
        ...
        print(f'{proxy}|{proxy_type}', file=open('good_proxy.txt', 'a'))
